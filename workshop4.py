...
stjerne_grafik = ["s1.png", "s2.png", "s3.png", "s4.png", "s5.png", "s6.png"]
...
def rumskib_ramt():
    rumskib.liv -= 1
    if rumskib.liv == 0:
        skab_eksplosion(rumskib.pos)
    else:
        rumskib.ramt = True
        clock.schedule(aflys_ramt_rumskib, 1)


def rumskib_ramt_draw():
    screen.draw.text(
...
def game_over_draw():
    screen.draw.text("GAME OVER", midtop=(WIDTH // 2, HEIGHT // 2), fontsize=70)
    clock.unschedule(skab_ufo)
